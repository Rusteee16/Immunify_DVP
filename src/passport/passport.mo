...
actor class Passport(owner : Principal, passportName : Text, passportId : Text, nation : Text, gender : Text, vacName : Text, vacDate : Text, issueD : Text, image : [Nat8]) = this {
    ...
    //Get passport owner icp registration id
    public query func getOwner() : async Principal {
        return ownerId;
    };

    //Get passport name, no, nation, gender, vaccine name and vaccine date
    public query func getPassport() : async DVP {
        return newDVP;
    };

    //Get Passport nft issue date
    ...
    //transfering ownership while sending or approving/rejecting the passport
    public shared (msg) func transferOwnership(caller : Principal, sharedOwner : Principal) : async Text {
        if (caller == ownerId) {
            ownerId := sharedOwner;
            return "Success";
        } else {
